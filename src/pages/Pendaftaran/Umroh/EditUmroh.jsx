...
import { apiEditUmroh, apiFetchUmrohDetail } from '@/services/umrohService';
import useAuthStore from '@/store/authStore';
import buildAntdFileFromUrl from '@/utils/buildAntdFileFromUrl';
...
const EditUmrohPage = () => {
  ...
  const editJamaahUmrohMutation = useMutation({
    mutationFn: apiEditUmroh,
    onSuccess: (data) => {
      reset();
      setPriceState(0)
      setEquipmentPriceState(0)
      queryClient.invalidateQueries(['umroh-jamaah']);
      setOpenResult({
        open: true,
        title: 'Data Jamaah Diperbarui',
        subtitle: `Data pendaftaran umroh atas nama "${data.data.registerName}" telah berhasil diperbarui.`,
      });
    },
  });

  const { data: resJamaah } = useQuery({
  ...
  const onSubmit = (data) => {
    ...
    editJamaahUmrohMutation.mutate(newData)
  }

  useEffect(() => {
    if (resUmrohDetail?.data) {
      const { registerId: _, birthDate, photoIdentity, selfPhoto, ...values } = resUmrohDetail.data;
      reset({
        ...values,
  ...
  return (
    <>
      ...
      <ResultSuccess
        ...
        extra={
          <Button type="primary" key="console" onClick={() => navigate(`/pendaftaran/umroh/${resUmrohDetail?.data?.umrohCode}/jamaah`)}>
            List Jamaah Umroh
          </Button>
        }
      />
      <Spin spinning={isFetchingLocation} size='large' fullscreen />
    </>

  )
}

export default EditUmrohPage
